export const buffs = {
    ...
    角斗士的终幕礼4: {
        check: ({ weaponType }: any) => ['单手剑', '双手剑', '长柄武器'].includes(weaponType),
    ...
    流浪大地的乐团4: {
        check: ({ weaponType }: any) => ['法器', '弓'].includes(weaponType),
    ...
    炽烈的炎之魔女4: {
        check: ({ element }: any) => element === '火',
        title: '魔女4：蒸发、融化伤害提高15%，[buffCount]层额外提高[dmg]%火元素伤害加成',
        data: {
            zf: 15,
            rh: 15,
            dmg: ({ params }: any) => (params.monv || 1) * 7.5,
            buffCount: ({ params }: any) => params.monv || 1
        }
    },
    昔日宗室之仪2: {
    ...
    昔日宗室之仪4: {
        ...
        check: ({ currentTalent }: any) => !currentTalent || currentTalent === 'q',
        data: {
            atkPct: 20
        }
    },
    ...
    绝缘之旗印4: {
        ...
        data: {
            qDmg: ({ attr }: any) => Math.min(75, (attr.reCharge.base + attr.reCharge.plus) * 0.25)
        }
    },
    华馆梦醒形骸记4: {
    ...
    来歆余响4: {
        ...
        data: {
            aPlus: ({ attr }: any) => (attr.atk.base + attr.atk.plus + attr.atk.pct * attr.atk.base / 100) * 0.7
        }
    },
    被怜爱的少女4: {
