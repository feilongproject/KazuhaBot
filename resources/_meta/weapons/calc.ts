const step = function (start: number, step = 0) {
    if (!step) {
        step = start / 4;
    }
    const ret: number[] = [];
...
const polearm = {
    ...
    "黑岩刺枪": {
        ...
        data: {
            atkPct: ({ refine }: any) => step(12)[refine] * 3
        }
    },
    "决斗之枪": {
    ...
    "匣里灭辰": {
        check: ({ element }: any) => ['水', '火'].includes(element),
    ...
    "护摩之杖": {
        ...
        data: {
            atkPlus: ({ attr, refine, calc }: any) => {
                const totalHp = calc(attr.hp);
    ...
    "薙草之稻光": [{
        ...
        sort: 10,
        data: {
            atkPct: ({ attr, refine }: any) => {
                const reCharge = attr.reCharge.base + attr.reCharge.plus - 100
...
const catalyst = {
    ...
    "魔导绪论": {
        check: ({ element }: any) => ["水", "雷"].includes(element),
    ...
    "不灭月华": {
        ...
        data: {
            aPlus: ({ attr, calc, refine }: any) => calc(attr.hp) * step(1, 0.5)[refine] / 100
        }
    },
    "神乐之真意": {
...
const sword = {
    "辰砂之纺锤": {
        ...
        data: {
            ePlus: ({ attr, calc, refine }: any) => calc(attr.def) * step(40)[refine] / 100
        }
    },
    "腐殖之剑": {
    ...
    "匣里龙吟": {
        check: ({ element }: any) => ['火', '雷'].includes(element),
    ...
    "冷刃": {
        check: ({ element }: any) => ['水', '冰'].includes(element),
    ...
    "磐岩结绿": [{
        ...
        data: {
            atkPlus: ({ attr, calc, refine }: any) => calc(attr.hp) * step(1.2)[refine] / 100
    ...
    "天空之刃": [{
        ...
        data: {
            aPlus: ({ attr, calc, refine }: any) => calc(attr.atk) * step(20)[refine] / 100,
            a2Plus: ({ attr, calc, refine }: any) => calc(attr.atk) * step(20)[refine] / 100
        }
    }],
    "风鹰剑": {
        title: "攻击力提高[_atkPct]%",
...
const claymore = {
    ...
    "赤角石溃杵": {
        ...
        data: {
            aPlus: ({ attr, calc, refine }: any) => calc(attr.def) * step(40)[refine] / 100,
            a2Plus: ({ attr, calc, refine }: any) => calc(attr.def) * step(40)[refine] / 100
        },
    }
};
